...
**Design note — a deliberate improvement on the spec.** The spec synthesizes a frame only when "the inject mask is non-zero or has just changed". That is not enough: the parser's first checksum-valid frame only *primes* the button baseline, so with no keyboard attached the very first injected press would be swallowed by priming and never produce a press edge. This task instead synthesizes an **idle heartbeat** whenever no real frame has arrived for 50 ms, regardless of mask. The parser primes at boot, injection always produces real edges, and a keyboard unplugged mid-session correctly releases its buttons. When a real keyboard is streaming, no synthetic frame is ever emitted.
