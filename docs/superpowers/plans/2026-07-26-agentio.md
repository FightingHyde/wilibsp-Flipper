...
**This is the first task where the shadow allocation is observable** (moved here from Task 6 on 2026-07-26: `agentio.c` lives in a static library, so until `st7796.c` references it the linker drops the object and no PSRAM is allocated). Confirm it now appears with the harness ON:

```bash
arm-none-eabi-size -A build/apps/hello_display/hello_display.elf | grep psram
```
Expected: `.psram_noload` is at least 307200 bytes (the shadow framebuffer).

Then confirm the taps cost nothing when the harness is off:
```bash
cmake --preset target -DFW2_AGENTIO=OFF
cmake --build --preset target --target hello_display
arm-none-eabi-size -A build/apps/hello_display/hello_display.elf | grep psram
python tools/fw.py configure --clean
```
Expected: `.psram_noload` is 0 or absent in the OFF build.
