...
Expected: all three builds succeed. Confirm the shadow really is in PSRAM and only in the ON build:

```bash
arm-none-eabi-size -A build/apps/hello_display/hello_display.elf | grep psram
```

Expected (FW2_AGENTIO=ON): `.psram_noload` is at least 307200 bytes. With OFF it is 0 or absent.
...
python tools/fw.py configure --clean
```
Expected: no `.psram_noload` allocation in the OFF build.
