...
**Do not assert on `.bss` growth here** (corrected 2026-07-26, same cause as the PSRAM assertion moved out of Tasks 6 and 7). No app calls `agentio_init()` until Task 11, so `--gc-sections` strips the entire capture path and the scratch buffers with it — the figures are unchanged from before this task. Measured on this branch: `hello_keyboard` still reports `.bss` 4280 and `.psram_noload` 307200 after this task lands.

The isolated cost of this task's buffers is +1948 bytes of `.bss` (`s_row` + `s_enc` = 1931 bytes plus alignment), confirmed by temporarily wiring `agentio_init()` into an app and reverting. **The real SRAM assertion lives in Task 11**, where the app wiring makes it observable.
