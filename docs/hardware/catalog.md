...
|---|---|---|
| NFC — ST25R3916B | I2C1 (SDA=26/SCL=27; address in the firmware's `nfc/hardware/st25r3916.h`) | **Implemented upstream** in `freewili-firmware`'s display core (`fwgui_nfc` + `rpNFC`, FwGUI RPC 0x6F–0x71, events 52 `nfcSnapshot` / 53 `nfcText`). See "Implemented upstream" below |
...
implemented in the default FreeWili 2 firmware (`freewili-firmware`), and
...
|---|---|---|
| LoRa — WIO-E5 bridge | DISPLAY core: `fwLoraBridge` + FwGUI RPC 0x66–0x6A; bridge fw in `freewiliwioe5` ("wiligo"); pairs with a Meshtastic display-firmware fork (`Ytuf/firmware`) | this repo `docs/drivers/lora.md`; firmware `agents/firmware/lora.md` |
| NFC — ST25R3916B | DISPLAY core: `fwgui_nfc` + `rpNFC`, FwGUI RPC 0x6F–0x71, events 52 `nfcSnapshot` / 53 `nfcText`, MIFARE Crypto1 on-device | firmware `agents/firmware/nfc.md` |
| Wi-Fi/BLE — ESP32-C5 (Bottlenose) | MAIN core: `rpBottleNoseOrca` over the PIO-UART (runtime UART TX 32/34, RX 33/35); no public standalone ESP32 repo — firmware ships as a release asset | firmware `agents/firmware/pio-allocation.md` (zone 5) |
| CM0 Linux module (BCM2837) | MAIN/FPGA-side: bridge daemon + OneWili C/Python in the firmware repo's `freewilicm0/` | firmware `agents/firmware/cm0-bridge.md` |
| Automatic power-zone manager | DISPLAY core: `rpZoneManager` family; host-tested | this repo `docs/drivers/power.md` |

Harvesting the LoRa bridge or NFC driver into this BSP would follow the
normal "add a driver" procedure with `freewili-firmware` as the source repo.
